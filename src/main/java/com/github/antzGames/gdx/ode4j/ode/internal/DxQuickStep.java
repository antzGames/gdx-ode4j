...
import com.github.antzGames.gdx.ode4j.ode.internal.joints.Info2DescrQuickStep;
...
import com.github.antzGames.gdx.ode4j.ode.internal.processmem.DxStepperProcessingCallContext.dmaxcallcountestimate_fn_t;
import com.github.antzGames.gdx.ode4j.ode.internal.processmem.DxStepperProcessingCallContext.dstepper_fn_t;
import com.github.antzGames.gdx.ode4j.ode.threading.Atomics;

import java.util.Arrays;
import java.util.concurrent.atomic.AtomicInteger;

import static com.github.antzGames.gdx.ode4j.ode.DMatrix.dSetValue;
import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dInvertMatrix3;
...
import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dMultiply2_333;
import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dMultiplyAdd0_331;
import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dSetCrossMatrixMinus;
import static com.github.antzGames.gdx.ode4j.ode.internal.Common.dFabs;
import static com.github.antzGames.gdx.ode4j.ode.internal.Common.dIASSERT;
import static com.github.antzGames.gdx.ode4j.ode.internal.Common.dIVERIFY;
import static com.github.antzGames.gdx.ode4j.ode.internal.Common.dRecip;
import static com.github.antzGames.gdx.ode4j.ode.internal.Matrix.dSetZero;
import static com.github.antzGames.gdx.ode4j.ode.internal.Misc.dRandInt;
import static com.github.antzGames.gdx.ode4j.ode.internal.QuickStepEnums.*;
...
public class DxQuickStep extends AbstractStepper implements dstepper_fn_t,
        DxWorldProcessIslandsInfo.dmemestimate_fn_t, dmaxcallcountestimate_fn_t {
...
//		for(i=0, k = 0; i<q; i++, k += 12)
	...
	private static class dxQuickStepperStage1CallContext
	{
		void Initialize(DxStepperProcessingCallContext stepperCallContext,
                        DxUtil.BlockPointer stageMemArenaState, double[] invI, DJointWithInfo1[] jointinfos)
		{
    ...
    private static class dxQuickStepperStage5CallContext {
        void Initialize(DxStepperProcessingCallContext callContext, dxQuickStepperLocalContext localContext,
                DxUtil.BlockPointer stage3MemArenaState) {
            m_stepperCallContext = callContext;
            m_localContext = localContext;
...
//		int findex;
	...
	{
		if (TIMING) Timer.dTimerStart("preprocessing");
...
//	#endif
	...
	void dxQuickStepIsland_Stage2a(dxQuickStepperStage2CallContext stage2CallContext)
	{
	    ...
	    final double stepsizeRecip = dRecip(callContext.m_stepSize());
	    {
	        ...
	        if (TIMING) Timer.dTimerNow ("create J");
	        ...
	        while ((ji = Atomics.ThrsafeIncrementIntUpToLimit(stage2CallContext.m_ji_J, nj)) != nj) {
	            ...
	            for (int jCurr = jRow; jCurr != jEnd; jCurr += JME__MAX) {
	            	...
	            	J[jCurr + JME_LO] = -OdeConstants.dInfinity;
	            	J[jCurr + JME_HI] = OdeConstants.dInfinity;
	            }
	            Jinfo.setAllP(jRow + JME__J1_MIN, jRow + JME__J2_MIN, jRow + JME__RHS_CFM_MIN, jRow + JME__LO_HI_MIN, ofsi);
	            DMatrix.dSetValue(findex, ofsi, infom, -1);
	...
	void dxQuickStepIsland_Stage2b(dxQuickStepperStage2CallContext stage2CallContext)
	{
	    ...
	    final double stepsizeRecip = dRecip(callContext.m_stepSize());
	    {
	        ...
	        if (TIMING) Timer.dTimerNow ("compute rhs_tmp");
	        ...
	        while ((bi_step = Atomics.ThrsafeIncrementIntUpToLimit(stage2CallContext.m_bi, nb_steps)) != nb_steps) {
	            ...
	            while (true) {
		            ...
		            for (int j = CommonEnums.dSA__MIN; j< CommonEnums.dSA__MAX; ++j)
		            	rhs_tmp[rhscurr + RHS__L_MIN + j] = -(b.facc.get(CommonEnums.dV3E__AXES_MIN + j) * body_invMass + b.lvel.get(CommonEnums.dV3E__AXES_MIN + j) * stepsizeRecip);
		            dMultiply0_331 (rhs_tmp, rhscurr + RHS__A_MIN, invI, invIrow + IIE__MATRIX_MIN, b.tacc);
		            for (int k = CommonEnums.dSA__MIN; k< CommonEnums.dSA__MAX; ++k)
		            	rhs_tmp[rhscurr + RHS__A_MIN + k] = -(b.avel.get(CommonEnums.dV3E__AXES_MIN + k) * stepsizeRecip) - rhs_tmp[rhscurr + RHS__A_MIN + k];
	                if (++bi == biend) {
	                    break;
	                }
	                rhscurr += RHS__MAX;
    ...
    void dxQuickStepIsland_Stage3(dxQuickStepperStage3CallContext stage3CallContext)
    {
	    ...
	    DxUtil.BlockPointer stage3MemarenaState = memarena.SaveState();
    ...
    void dxQuickStepIsland_Stage4LCP_AdComputation(dxQuickStepperStage4CallContext stage4CallContext)
    {
        ...
        DxWorld world = callContext.m_world();
        Objects_H.dxQuickStepParameters qs = world.qs;
    ...
    void dxQuickStepIsland_Stage4b(dxQuickStepperStage4CallContext stage4CallContext)
    {
        ...
        if (Atomics.ThrsafeExchange(stage4CallContext.m_cf_4b, 1) == 0) {
            ...
            for (int bodycurr = 0; bodycurr != bodyend; cforcecurr += CFE__MAX, bodycurr++) {
                ...
                for (int j = CommonEnums.dSA__MIN; j != CommonEnums.dSA__MAX; j++) {
                    b.lvel.add(CommonEnums.dV3E__AXES_MIN + j, stepsize * cforce[cforcecurr + CFE__L_MIN + j]);
                    b.avel.add(CommonEnums.dV3E__AXES_MIN + j, stepsize * cforce[cforcecurr + CFE__A_MIN + j]);
                }
            }
        }
