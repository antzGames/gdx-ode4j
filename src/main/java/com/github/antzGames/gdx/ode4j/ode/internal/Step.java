...
import com.github.antzGames.gdx.ode4j.ode.DMatrix;
import com.github.antzGames.gdx.ode4j.ode.OdeConstants;
import com.github.antzGames.gdx.ode4j.ode.OdeMath;
import com.github.antzGames.gdx.ode4j.ode.internal.joints.DxJoint;
import com.github.antzGames.gdx.ode4j.ode.internal.joints.DxJointNode;
import com.github.antzGames.gdx.ode4j.ode.internal.joints.Info2DescrStep;
import com.github.antzGames.gdx.ode4j.ode.internal.processmem.DxStepperProcessingCallContext;
import com.github.antzGames.gdx.ode4j.ode.internal.processmem.DxUtil;
import com.github.antzGames.gdx.ode4j.ode.internal.processmem.DxWorldProcessIslandsInfo;
import com.github.antzGames.gdx.ode4j.ode.internal.processmem.DxWorldProcessMemArena;
import com.github.antzGames.gdx.ode4j.ode.threading.Atomics;

import java.util.concurrent.atomic.AtomicInteger;

import static com.github.antzGames.gdx.ode4j.ode.DMatrix.dSetZero;
import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dMultiply0_133;
import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dMultiply0_331;
import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dMultiply0_333;
import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dMultiplyAdd0_331;
import static com.github.antzGames.gdx.ode4j.ode.internal.Matrix.dSetValue;

class Step extends AbstractStepper implements DxStepperProcessingCallContext.dstepper_fn_t, DxWorldProcessIslandsInfo.dmemestimate_fn_t,
        DxStepperProcessingCallContext.dmaxcallcountestimate_fn_t {
	...
	private static class dxStepperStage1CallContext
	{
		dxStepperStage1CallContext(final DxStepperProcessingCallContext stepperCallContext,
                                   DxUtil.BlockPointer stageMemArenaState, double[] invI,
	...
	private static class dxStepperStage3CallContext
	{
		void Initialize(final DxStepperProcessingCallContext callContext,
				...
				DxUtil.BlockPointer stage1MemArenaState)
		{
...
//	}
	...
	private void dxStepIsland(final DxStepperProcessingCallContext callContext)
	{
		...
		Common.dIASSERT(allowedThreads != 0);

		DxUtil.BlockPointer stagesMemArenaState = memarena.SaveState();
...
//			}
	...
	void dxStepIsland_Stage0_Bodies(dxStepperStage0BodiesCallContext callContext)
	{
		...
		// accumulator. I and invI are a vertical stack of 3x4 matrices, one per body.
		{
			...
			int bodyIndex = Atomics.ThrsafeIncrementIntUpToLimit(callContext.m_inertiaBodyIndex, nb);
			...
			for (int i = 0; i != nb; invIrowO += 12, ++i) {
				if (i == bodyIndex) {
					...
					OdeMath.dMultiply0_333 (invIrowA, invIrowO,b.posr().R(),tmp);
					...
					if (b.isFlagsGyroscopic() && (b.invMass>0)) {
						...
						OdeMath.dMultiply0_331(L,I,b.avel);
						...
						OdeMath.dSetCrossMatrixMinus(Itild,L);//,4);
						...
						// a sort of "torque"
						L.scale(Common.dRecip(h));//dScaleVector3(L,dRecip(h));
						...
						if (OdeMath.dInvertMatrix3(itInv,Itild)!=0) {
							// "Divide" the original tensor
							// by the pseudo-tensor (on the right)
							OdeMath.dMultiply0_333(Itild,I,itInv);
							...
							OdeMath.dMultiply0_331(tau0,Itild,L);
					...
					bodyIndex = Atomics.ThrsafeIncrementIntUpToLimit(callContext.m_inertiaBodyIndex, nb);
				}
			}
		}
	}
			...
			while (true) {
				...
				// Switch to growing array forward
				{
					...
					while (true) {  // jicurr=dest, _jcurr=src
						...
						Common.dIASSERT (/*jicurr->info.m >= 0 && */jicurrO.info.m <= 6 && /*jicurr->info.nub >= 0 && */jicurrO.info.nub <= jicurrO.info.m);
						if (jicurrO.info.m > 0) {
	...
	void dxStepIsland_Stage1(dxStepperStage1CallContext stage1CallContext)
	{
		...
		DxWorldProcessMemArena memarena = callContext.m_stepperArena();
		{
			...
			Common.dIVERIFY(stage1CallContext == null); // To suppress compiler warnings about unused variable assignment
		...
		DxUtil.BlockPointer stage1MemarenaState = memarena.SaveState();
	...
	void dxStepIsland_Stage2a(dxStepperStage2CallContext stage2CallContext)
	{
		...
		final double stepsizeRecip = Common.dRecip(callContext.m_stepSize());
		DxWorld world = callContext.m_world();

		{
			...
			while ((ji = Atomics.ThrsafeIncrementIntUpToLimit(stage2CallContext.m_ji_J, nj)) != nj) {
				...
				Jinfo.J2lp = J2rowP;
				Jinfo.J2ap = J2rowP + 4;
				DMatrix.dSetZero (J, J1rowP, 2*8*infom);
				...
				//Jinfo.c = rhs + ofsi;
				DMatrix.dSetZero(rhs, ofsi, infom);//dSetZero (rhs, Jinfo.c, infom);
				//Jinfo.cfm = cfm + ofsi;
				Matrix.dSetValue(cfm, ofsi, infom, world.global_cfm);//dSetValue(Jinfo.cfm, infom, world.global_cfm);
				//Jinfo.lo = lo + ofsi;
				Matrix.dSetValue (lo, ofsi, infom, -OdeConstants.dInfinity);
	            //Jinfo.hi = hi + ofsi;
				Matrix.dSetValue (hi, ofsi, infom, OdeConstants.dInfinity);
	            //Jinfo.findex = findex + ofsi;
	            //Matrix.dSetValue(findex, ofsi, infom, -1);
	...
	void dxStepIsland_Stage2b(dxStepperStage2CallContext stage2CallContext)
	{
...

		{
			...
			final int mskip = Common.dPAD(m);

			int ji;
			while ((ji = Atomics.ThrsafeIncrementIntUpToLimit(stage2CallContext.m_ji_Ainit, nj)) != nj) {
				final int ofsi = mindex[ji];
				final int infom = mindex[ji + 1] - ofsi;

				double[] ArowA = A;
				int ArowP = mskip*ofsi;
				DMatrix.dSetZero(ArowA, ArowP, mskip*infom);
				double[] AdiagA = ArowA;
...

		{
			...
			int ji;
			while ((ji = Atomics.ThrsafeIncrementIntUpToLimit(stage2CallContext.m_ji_JinvM, nj)) != nj) {
				final int ofsi = mindex[ji];
				final int infom = mindex[ji + 1] - ofsi;

				int Jdst = 2*8*ofsi;//JinvM + 2*8*ofsi;

				DMatrix.dSetZero(JinvM, Jdst, 2*8*infom);
...

		{
			...
			int bi;
			while ((bi = Atomics.ThrsafeIncrementIntUpToLimit(stage2CallContext.m_bi_rhs_tmp, nb)) != nb) {
				...
				OdeMath.dMultiply0_331 (tmp1currA, tmp1currP+4, invIrowA, invIrowP, b.tacc);
	...
	void dxStepIsland_Stage2c(dxStepperStage2CallContext stage2CallContext)
	{
...

		{
			...
			int ji;
			while ((ji = Atomics.ThrsafeIncrementIntUpToLimit(stage2CallContext.m_ji_Aaddjb, nj)) != nj) {
				final int ofsi = mindex[ji];
				final int infom = mindex[ji + 1] - ofsi;
...

		{
			...
			int ji;
			while ((ji = Atomics.ThrsafeIncrementIntUpToLimit(stage2CallContext.m_ji_rhs, nj)) != nj) {
				final int ofsi = mindex[ji];
				final int infom = mindex[ji + 1] - ofsi;
	...
	void dxStepIsland_Stage3(dxStepperStage3CallContext stage3CallContext)
	{
		...
		Common.dIVERIFY(stage3CallContext == null); // To suppress unused variable assignment warnings
