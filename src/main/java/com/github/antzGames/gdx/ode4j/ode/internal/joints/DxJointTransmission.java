...
import com.github.antzGames.gdx.ode4j.ode.DTransmissionJoint;
import com.github.antzGames.gdx.ode4j.ode.OdeConstants;
import com.github.antzGames.gdx.ode4j.ode.internal.DxWorld;

import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dAddScaledVectors3;
...
import static com.github.antzGames.gdx.ode4j.ode.internal.Common.M_PI_2;
...
//                u[2] = radii[i] * sinphi;
        ...
        if (backlash == 0 || Math.abs(delta) > backlash) {
            ...
            if (delta > 0) {
                if (backlash > 0) {
                    info.setLo(0, -OdeConstants.dInfinity);
                    info.setHi(0, 0);
                }
...
//                                result);
    ...
    void dJointSetTransmissionParam( PARAM parameter, double value )
    {
        switch ( parameter ) {
            case dParamCFM:
                cfm = value;
    ...
    double dJointGetTransmissionParam( PARAM parameter )
    {
        switch ( parameter ) {
            case dParamCFM:
                return cfm;
