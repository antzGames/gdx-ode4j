...
import com.github.antzGames.gdx.ode4j.ode.DAABB;
...
import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dMultiply1_333;
import static com.github.antzGames.gdx.ode4j.ode.internal.Common.dCeil;
import static com.github.antzGames.gdx.ode4j.ode.internal.Common.dEpsilon;
import static com.github.antzGames.gdx.ode4j.ode.internal.Common.dFabs;
import static com.github.antzGames.gdx.ode4j.ode.internal.Common.dFloor;
	...
	void computeAABB()
	{
		...
		if ( d.m_bWrapMode == false )
		{
			...
			if (hasFlagPlaceable())
			{
				...
				// Y-axis
	            if (d.m_fMinHeight != -OdeConstants.dInfinity)
	            {
	                ...
	                dy[0] = ( final_posr().R().get01()!=0 ? final_posr().R().get01() * -OdeConstants.dInfinity : (0.0) );
	                dy[1] = ( final_posr().R().get11()!=0 ? final_posr().R().get11() * -OdeConstants.dInfinity : (0.0) );
	                dy[2] = ( final_posr().R().get21()!=0 ? final_posr().R().get21() * -OdeConstants.dInfinity : (0.0) );
	            }

	            if (d.m_fMaxHeight != OdeConstants.dInfinity)
	            {
				dy[3] = ( final_posr().R().get01() * d.m_fMaxHeight );
				dy[4] = ( final_posr().R().get11() * d.m_fMaxHeight );
				dy[5] = ( final_posr().R().get21() * d.m_fMaxHeight );
	            } else {
	                dy[3] = ( final_posr().R().get01()!=0 ? final_posr().R().get01() * OdeConstants.dInfinity : (0.0) );
	                dy[4] = ( final_posr().R().get11()!=0 ? final_posr().R().get11() * OdeConstants.dInfinity : (0.0) );
	                dy[5] = ( final_posr().R().get21()!=0 ? final_posr().R().get21() * OdeConstants.dInfinity : (0.0) );
	            }
		...
		else
		{
			...
			if (hasFlagPlaceable())
			{
				...
				_aabb.set(-OdeConstants.dInfinity, +OdeConstants.dInfinity,
						-OdeConstants.dInfinity, +OdeConstants.dInfinity,
						-OdeConstants.dInfinity, +OdeConstants.dInfinity);
...
//}
			...
			int skip )
	{
		...
		//unsigned 
		int x_local, z_local;
		double maxY = - OdeConstants.dInfinity;
		double minY = OdeConstants.dInfinity;
			...
			if (!dCollideHeightfieldExit) {
			    {
			        ...
			        int nMinX = (int)dFloor(Common.dNextAfter(o2._aabb.getMin0() * fInvSampleWidth, -OdeConstants.dInfinity));
			        int nMaxX = (int)dCeil(Common.dNextAfter(o2._aabb.getMax0() * fInvSampleWidth, OdeConstants.dInfinity));
			        final double fInvSampleDepth = terrain.m_p_data.m_fInvSampleDepth;
			        int nMinZ = (int)dFloor(Common.dNextAfter(o2._aabb.getMin2() * fInvSampleDepth, -OdeConstants.dInfinity));
			        int nMaxZ = (int)dCeil(Common.dNextAfter(o2._aabb.getMax2() * fInvSampleDepth, OdeConstants.dInfinity));
