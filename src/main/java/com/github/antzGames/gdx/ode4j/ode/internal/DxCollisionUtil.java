...
import com.github.antzGames.gdx.ode4j.ode.DContactGeom;
...
import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dCalcVectorCross3;
...
//	}
	...
	final DContactGeom[] SAFECONTACT(int Flags, DContactGeom[] Contacts, int Index, int Stride){
		Common.dIASSERT(Index >= 0 && Index < (Flags & DxGeom.NUMC_MASK));
...
//			rZ.set(0.0);
//		}
		rY.set( Math.asin(mRot.get02()) );
		if (rY.get() < Common.M_PI /2)
		{
			if (rY.get() > -Common.M_PI /2)
			{
				rX.set( Math.atan2(-mRot.get12(), mRot.get22()) );
				rZ.set( Math.atan2(-mRot.get01(), mRot.get00()) );
			}
			else
			{
				// not unique
				rX.set( -Math.atan2(mRot.get10(), mRot.get11()) );
				rZ.set(0.0);
			}
		}
		else
...
//		p.v[2] = pb.get2() - pa.get2();
			...
			DVector3 cp1, DVector3 cp2)
	{
		...
		SET3_SUB (b1b2,b2,b1);
		SET3_SUB (a1b1,b1,a1);
		da1 = OdeMath.dCalcVectorDot3(a1a2,a1b1);
		db1 = OdeMath.dCalcVectorDot3(b1b2,a1b1);
		if (da1 <= 0 && db1 >= 0) {
			SET2 (cp1,a1);
			SET2 (cp2,b1);
			return;
		...
		SET3_SUB (a1b2,b2,a1);
		da2 = OdeMath.dCalcVectorDot3(a1a2,a1b2);
		db2 = OdeMath.dCalcVectorDot3(b1b2,a1b2);
		if (da2 <= 0 && db2 <= 0) {
			SET2 (cp1,a1);
			SET2 (cp2,b2);
			return;
		...
		SET3_SUB (a2b1,b1,a2);
		da3 = OdeMath.dCalcVectorDot3(a1a2,a2b1);
		db3 = OdeMath.dCalcVectorDot3(b1b2,a2b1);
		if (da3 >= 0 && db3 >= 0) {
			SET2 (cp1,a2);
			SET2 (cp2,b1);
			return;
		...
		SET3_SUB (a2b2,b2,a2);
		da4 = OdeMath.dCalcVectorDot3(a1a2,a2b2);
		db4 = OdeMath.dCalcVectorDot3(b1b2,a2b2);
		if (da4 >= 0 && db4 <= 0) {
			SET2 (cp1,a2);
			SET2 (cp2,b2);
			return;
		...
		la = OdeMath.dCalcVectorDot3(a1a2,a1a2);
		if (da1 >= 0 && da3 <= 0) {
			k = da1 / la;
			SET3_SUB (n,1,a1b1,k,a1a2);
			if (OdeMath.dCalcVectorDot3(b1b2,n) >= 0) {
		...
		lb = OdeMath.dCalcVectorDot3(b1b2,b1b2);
		if (db1 <= 0 && db2 >= 0) {
			k = -db1 / lb;
			SET3_SUB (n,-1,a1b1,k,b1b2);
			if (OdeMath.dCalcVectorDot3(a1a2,n) >= 0) {
				SET2 (cp1,a1);
		...
		k = OdeMath.dCalcVectorDot3(a1a2,b1b2);
		det = la*lb - k*k;
		if (det <= 0) {
...
//		h.v[2] = (0.5) * side.get2();
	...
	private static void answer(final double t, DVector3 tmp, DVector3C sign,
		...
		OdeMath.dMultiply0_331 (s,R,tmp);
			...
			final DMatrix3 R2, final DVector3 side2)
	{
		...
		p.eqDiff(p2, p1);
		OdeMath.dMultiply1_331 (pp,R1,p);		// get pp = p relative to body 1
		...
		R11 = OdeMath.dCalcVectorDot3_44(R1,0,R2,0); R12 = OdeMath.dCalcVectorDot3_44(R1,0,R2,1); R13 = OdeMath.dCalcVectorDot3_44(R1,0,R2,2);
		Q11 = Common.dFabs(R11); Q12 = Common.dFabs(R12); Q13 = Common.dFabs(R13);
		if (Common.dFabs(pp.get0()) > (A1 + B1*Q11 + B2*Q12 + B3*Q13)) return false;
		R21 = OdeMath.dCalcVectorDot3_44(R1,1,R2,0); R22 = OdeMath.dCalcVectorDot3_44(R1,1,R2,1); R23 = OdeMath.dCalcVectorDot3_44(R1,1,R2,2);
		Q21 = Common.dFabs(R21); Q22 = Common.dFabs(R22); Q23 = Common.dFabs(R23);
		if (Common.dFabs(pp.get1()) > (A2 + B1*Q21 + B2*Q22 + B3*Q23)) return false;
		R31 = OdeMath.dCalcVectorDot3_44(R1,2,R2,0); R32 = OdeMath.dCalcVectorDot3_44(R1,2,R2,1); R33 = OdeMath.dCalcVectorDot3_44(R1,2,R2,2);
		Q31 = Common.dFabs(R31); Q32 = Common.dFabs(R32); Q33 = Common.dFabs(R33);
		if (Common.dFabs(pp.get2()) > (A3 + B1*Q31 + B2*Q32 + B3*Q33)) return false;

		// separating axis = v1,v2,v3
		if (Common.dFabs(OdeMath.dCalcVectorDot3_41(R2,0,p)) > (A1*Q11 + A2*Q21 + A3*Q31 + B1)) return false;
		if (Common.dFabs(OdeMath.dCalcVectorDot3_41(R2,1,p)) > (A1*Q12 + A2*Q22 + A3*Q32 + B2)) return false;
		if (Common.dFabs(OdeMath.dCalcVectorDot3_41(R2,2,p)) > (A1*Q13 + A2*Q23 + A3*Q33 + B3)) return false;

		// separating axis = u1 x (v1,v2,v3)
		if (Common.dFabs(pp.get2()*R21-pp.get1()*R31) > A2*Q31 + A3*Q21 + B2*Q13 + B3*Q12) return false;
		if (Common.dFabs(pp.get2()*R22-pp.get1()*R32) > A2*Q32 + A3*Q22 + B1*Q13 + B3*Q11) return false;
		if (Common.dFabs(pp.get2()*R23-pp.get1()*R33) > A2*Q33 + A3*Q23 + B1*Q12 + B2*Q11) return false;

		// separating axis = u2 x (v1,v2,v3)
		if (Common.dFabs(pp.get0()*R31-pp.get2()*R11) > A1*Q31 + A3*Q11 + B2*Q23 + B3*Q22) return false;
		if (Common.dFabs(pp.get0()*R32-pp.get2()*R12) > A1*Q32 + A3*Q12 + B1*Q23 + B3*Q21) return false;
		if (Common.dFabs(pp.get0()*R33-pp.get2()*R13) > A1*Q33 + A3*Q13 + B1*Q22 + B2*Q21) return false;

		// separating axis = u3 x (v1,v2,v3)
		if (Common.dFabs(pp.get1()*R11-pp.get0()*R21) > A1*Q21 + A2*Q11 + B2*Q33 + B3*Q32) return false;
		if (Common.dFabs(pp.get1()*R12-pp.get0()*R22) > A1*Q22 + A2*Q12 + B1*Q33 + B3*Q31) return false;
		if (Common.dFabs(pp.get1()*R13-pp.get0()*R23) > A1*Q23 + A2*Q13 + B1*Q32 + B2*Q31) return false;
