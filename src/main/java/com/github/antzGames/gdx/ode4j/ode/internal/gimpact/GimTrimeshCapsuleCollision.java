...
//	#endif // GIM_TRI_CAPSULE_COLLISION_H_INCLUDED
	...
	*/
	...
	static void gim_closest_point_triangle_segment(GimTriCollision.GIM_TRIANGLE_DATA triangle, GimGeometry.vec3f s1, GimGeometry.vec3f s2,
                                                   GimDynArray<GimContact> contacts)
	{
	    GimGeometry.vec3f[] segment_points = { new GimGeometry.vec3f(), new GimGeometry.vec3f(), new GimGeometry.vec3f(), new GimGeometry.vec3f() };//new vec3f[4];// = {{0}};  //TZ TODO ???
	    GimGeometry.vec3f[] closest_points = { new GimGeometry.vec3f(), new GimGeometry.vec3f() };//new vec3f[2];// = {{0}};
	    int intersection_type, out_edge = 10;
	    float dis, dis_temp,perpend;
	    GimGeometry.vec4f sdiff = new GimGeometry.vec4f();

	    dis = GimGeometry.DISTANCE_PLANE_POINT(triangle.m_planes.m_planes[0],s1);
	    dis_temp = GimGeometry.DISTANCE_PLANE_POINT(triangle.m_planes.m_planes[0],s2);

	    if(dis<=0.0f && dis_temp<=0.0f) return;

	    GimGeometry.VEC_DIFF(sdiff,s2,s1);
	    perpend = GimGeometry.VEC_DOT(sdiff,triangle.m_planes.m_planes[0]);

	    if(!GimMath.IS_ZERO(perpend)) // Not perpendicular
	    {
	        if(dis<dis_temp)
	        {
	    ...
	    //    Edge1
	    	intersection_type = GimGeometry.PLANE_CLIP_SEGMENT_CLOSEST(s1,s2,triangle.m_planes.m_planes[1],
	        ...
	        {
	            //Edge2
	        	intersection_type = GimGeometry.PLANE_CLIP_SEGMENT_CLOSEST(segment_points[0],segment_points[1],triangle.m_planes.m_planes[2],
	            ...
	            {
	                //Edge3
	            	intersection_type = GimGeometry.PLANE_CLIP_SEGMENT_CLOSEST(segment_points[2],segment_points[3],triangle.m_planes.m_planes[3],
	        ...
	        {
	            dis = GimGeometry.DISTANCE_PLANE_POINT(triangle.m_planes.m_planes[0],closest_points[0]);
	            GimContact.GIM_PUSH_CONTACT(contacts,closest_points[0] ,triangle.m_planes.m_planes[0] ,dis,null,null, 0,0);
	    ...
	    for(i=0;i<3;i++)
	    {
	        ...
	        if(dis_temp< dis)
	        {
	            ...
	            out_edge = i;
	            GimGeometry.VEC_COPY(closest_points[0],segment_points[0]);
	            GimGeometry.VEC_COPY(closest_points[1],sdiff);//normal
	        }
	    }
	    ...
	    {
	        dis = GimMath.GIM_SQRT(dis);//GIM_SQRT(dis,dis);
	        GimGeometry.VEC_SCALE(closest_points[1],(1.0f/dis),closest_points[1]);//normal
	...
	static int gim_triangle_capsule_collision(GimTriCollision.GIM_TRIANGLE_DATA triangle, GIM_CAPSULE_DATA capsule,
                                              GimDynArray<GimContact> contacts)
	{
