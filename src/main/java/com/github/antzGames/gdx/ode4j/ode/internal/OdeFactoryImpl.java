...
import com.github.antzGames.gdx.ode4j.ode.DMisc;
...
public class OdeFactoryImpl extends OdeJointsFactoryImpl {
	...
	static void checkWorld (DxWorld w)
	{
		...
		if (listHasLoops (w.firstbody)) ErrorHandler.dDebug (0,"body list has loops");
		if (listHasLoops (w.firstjoint)) ErrorHandler.dDebug (0,"joint list has loops");
...
//		}
		...
		if (w.nb != nn) ErrorHandler.dDebug (0,"body count incorrect");
		nn = 0;
		for (j=w.firstjoint.get(); j!=null; j=(DxJoint)j.getNext()) nn++;
		if (w.nj != nn) ErrorHandler.dDebug (0,"joint count incorrect");
		...
		for (b=w.firstbody.get(); b!=null; b=(DxBody)b.getNext()) if (b.world != w)
			ErrorHandler.dDebug (0,"bad world pointer in body list");
		for (j=w.firstjoint.get(); j!=null; j=(DxJoint)j.getNext()) if (j.world != w)
			ErrorHandler.dDebug (0,"bad world pointer in joint list");
  ...
  for (j=w.firstjoint; j; j=(dxJoint*)j.next) {
	...
	dDebug (0,"half connected joint found");
		...
		for (j=w.firstjoint.get(); j!=null; j=(DxJoint)j.getNext()) {
			for (int i=0; i<2; i++) {
				if (j.node[i].body!=null) {
					...
					if (ok==0) ErrorHandler.dDebug (0,"joint not in joint list of attached body");
				}
			}
		}
			...
			for (DxJointNode n=b.firstjoint.get(); n!=null; n=n.next) {
				if (n.joint.node[0] == n) {
					if (n.joint.node[1].body != b)
						ErrorHandler.dDebug (0,"bad body pointer in joint node of body list (1)");
				}
				else {
					if (n.joint.node[0].body != b)
						ErrorHandler.dDebug (0,"bad body pointer in joint node of body list (2)");
				}
				if (n.joint.tag != count) ErrorHandler.dDebug (0,"bad joint node pointer in body");
			}
		}
		...
		for (j=w.firstjoint.get(); j!=null; j=(DxJoint)j.getNext()) {
			if (j.node[0].body!=null && (j.node[0].body == j.node[1].body))
				ErrorHandler.dDebug (0,"non-distinct body pointers in joint");
			if ((j.node[0].body!=null && j.node[0].body.tag != count) ||
					(j.node[1].body!=null && j.node[1].body.tag != count))
				ErrorHandler.dDebug (0,"bad body pointer in joint");
		}
	}
	...
	public void dTestDataStructures()
	{
		...
		for (;;) {
			...
			if (nj < NUM && nb > 2 && DMisc.dRandReal() > 0.5) {
				DxBody b1 = body [(int) (DMisc.dRand() % nb)];
				DxBody b2 = body [(int) (DMisc.dRand() % nb)];
				if (b1 != b2) {
					//DO(
			...
			if (nj > 0 && nb > 2 && DMisc.dRandReal() > 0.5) {
				DxBody b1 = body [(int) (DMisc.dRand() % nb)];
				DxBody b2 = body [(int) (DMisc.dRand() % nb)];
				if (b1 != b2) {
					int k = (int) (DMisc.dRand() % nj);
					//DO(
