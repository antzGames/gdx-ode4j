...
import com.github.antzGames.gdx.ode4j.ode.internal.cpp4j.java.IntArray;
import com.github.antzGames.gdx.ode4j.ode.internal.cpp4j.java.ObjArray;
import com.github.antzGames.gdx.ode4j.ode.internal.gimpact.GimTrimeshCapsuleCollision.GIM_CAPSULE_DATA;
...
public class GimTrimesh implements GimConstants {
	...
	GimDynArray<GimTriCollision.GIM_TRIPLANES_CACHE> m_planes_cache_buffer;//! Allocated GIM_TRIPLANES_CACHE
	...
	*/
	...
	public static GimDynArray<GimGeometry.vec4f> GIM_CREATE_TRIMESHPLANE_CONTACTS() {
		return GimDynArray.GIM_DYNARRAY_CREATE(GimDynArray.G_ARRAY_GROW_SIZE);//vec4f,dynarray,G_ARRAY_GROW_SIZE); 
	}
	...
	*/
	...
	public int gim_trimesh_ray_collision(GimGeometry.vec3f origin, GimGeometry.vec3f dir,
                                         final float tmax, GimTriCollision.GIM_TRIANGLE_RAY_CONTACT_DATA contact) {
	...
	*/
	...
	public int gim_trimesh_ray_closest_collision(
            GimGeometry.vec3f origin, GimGeometry.vec3f dir, float tmax, GimTriCollision.GIM_TRIANGLE_RAY_CONTACT_DATA contact) {
...
//	    return false;
	...
	*/
	...
	private final GimBufferArrayFloat.GIM_PROCESS_BUFFER_ARRAY_FN MULT_MAT_VEC4_KERNEL =
		new GimBufferArrayFloat.GIM_PROCESS_BUFFER_ARRAY_FN() {
		@Override
	...
	*/
	...
	void gim_trimesh_update_vertices()
	{
	    ...
	    GimGeometry.COPY_MATRIX_4X4(transform,m_transform);  //TODO TZ why copy?
	...
	*/
	...
	{
	    ObjArray<GimGeometry.vec3f> transformed_vertices = m_transformed_vertex_buffer.GIM_BUFFER_ARRAY_POINTER(0);
...
//	        COMPUTEAABB_FOR_TRIANGLE((paabb[i]),v1,v2,v3);
	        GimGeometry.COMPUTEAABB_FOR_TRIANGLE(m_aabbset.at(i),//paabb[i],
...
//	    if(IS_ZERO(diff)) return ;///don't need to update
	    ...
	    GimGeometry.COPY_MATRIX_4X4(m_transform,transform);
	...
	*/
	...
	void gim_trimesh_get_triangle_data(int triangle_index, 
			final GimTriCollision.GIM_TRIANGLE_DATA tri_data)
	{
	    ObjArray<GimGeometry.vec3f> transformed_vertices = m_transformed_vertex_buffer.GIM_BUFFER_ARRAY_POINTER(0);
	    ...
	    //Get the planes
	    ObjArray<GimTriCollision.GIM_TRIPLANES_CACHE> planes = m_planes_cache_buffer.GIM_DYNARRAY_POINTER_V();
	    planes.inc(triangle_index);
	    if (planes.at0()==null) planes.setAt0(new GimTriCollision.GIM_TRIPLANES_CACHE());
	    ...
	    GimGeometry.VEC_COPY_4( tri_data.m_planes.m_planes[0], plane.m_planes[0] );//face plane
	    GimGeometry.VEC_COPY_4( tri_data.m_planes.m_planes[1], plane.m_planes[1] );//edge1
	    GimGeometry.VEC_COPY_4( tri_data.m_planes.m_planes[2], plane.m_planes[2] );//edge2
	    GimGeometry.VEC_COPY_4( tri_data.m_planes.m_planes[3], plane.m_planes[3] );//edge3
	}
	...
	*/
	...
	public void gim_trimesh_get_triangle_vertices(
            int triangle_index, GimGeometry.vec3f v1, GimGeometry.vec3f v2, GimGeometry.vec3f v3)
	{
