...
import com.github.antzGames.gdx.ode4j.ode.internal.cpp4j.java.ObjArray;
import com.github.antzGames.gdx.ode4j.ode.internal.cpp4j.java.RefFloat;
...
//		}
//	}
	private static boolean CLASSIFY_TRI_BY_FACE(final GimGeometry.vec3f[] v, final GimGeometry.vec4f faceplane)
	{   
		float d0, d1, d2; 
	    d0 = GimGeometry.DISTANCE_PLANE_POINT(faceplane,v[0]);
	    d1 = d0 * GimGeometry.DISTANCE_PLANE_POINT(faceplane,v[1]);
	    d2 = d0 * GimGeometry.DISTANCE_PLANE_POINT(faceplane,v[2]);
		if( d1>0.0f && d2>0.0f ) {
		    return true;
		} else {
		    return false;
	...
	private static int MOST_DEEP_POINTS(final GimGeometry.vec4f plane, GimGeometry.vec3f[] points, final int point_count,
                                        final GimGeometry.vec3f[] deep_points, final RefFloat maxdeep, final int[] _max_candidates)
	{
	    maxdeep.d=-1000.0f;
	    int _k;
		float _dist;
		...
		{
		    _dist = -GimGeometry.DISTANCE_PLANE_POINT(plane,points[_k]);
			if(_dist>maxdeep.d)
			{
				maxdeep.d = _dist;
			...
			else if((_dist+ GimMath.G_EPSILON)>=maxdeep.d)
			{
	...
	private static int CLIP_TRI_POINTS_BY_TRI_EDGE_PLANES(final GimGeometry.vec3f[] tri_points,
                                                          final GimGeometry.vec4f[] tri_edge_planes, final int pos,
                                                          final GimGeometry.vec3f[] clipped_points, final GimGeometry.vec3f[] _temp_clip,
                                                          final GimGeometry.vec3f[] _temp_clip2)//RefInt clipped_point_count)
	{
...
//	    }
	    ...
	    _temp_clip_count = GimTriCollision.PLANE_CLIP_POLYGON(
	    		tri_edge_planes[0+pos], tri_points, 3, _temp_clip, GimTriCollision.MAX_TRI_CLIPPING);
	    if( _temp_clip_count > 0 )
	    {
	        int _temp_clip_count2 = 0;
	        _temp_clip_count2 = GimTriCollision.PLANE_CLIP_POLYGON(
	        		tri_edge_planes[1+pos], _temp_clip, _temp_clip_count, _temp_clip2, GimTriCollision.MAX_TRI_CLIPPING);
	        if( _temp_clip_count2 > 0 )
	        {
	        	clipped_point_count = GimTriCollision.PLANE_CLIP_POLYGON(
	        			tri_edge_planes[2+pos], _temp_clip2, _temp_clip_count2, clipped_points, GimTriCollision.MAX_TRI_CLIPPING);
	        }
	    }
...
//			GIM_TRIANGLE_CONTACT_DATA * contact_data)
	static boolean _gim_triangle_triangle_collision(
			final GimTriCollision.GIM_TRIANGLE_DATA tri1,
			final GimTriCollision.GIM_TRIANGLE_DATA tri2,
			final GimTriCollision.GIM_TRIANGLE_CONTACT_DATA contact_data)
	{
	    //Cache variables for triangle intersection
	    int[] _max_candidates = new int[GimTriCollision.MAX_TRI_CLIPPING];
	    GimGeometry.vec3f[] _temp_clip = new GimGeometry.vec3f[GimTriCollision.MAX_TRI_CLIPPING];
//	    int _temp_clip_count = 0;
	    GimGeometry.vec3f[] _temp_clip2 = new GimGeometry.vec3f[GimTriCollision.MAX_TRI_CLIPPING];
//	    int _temp_clip_count2 = 0;
	    GimGeometry.vec3f[] clipped_points2 = new GimGeometry.vec3f[GimTriCollision.MAX_TRI_CLIPPING];
	    GimGeometry.vec3f[] deep_points2 = new GimGeometry.vec3f[GimTriCollision.MAX_TRI_CLIPPING];
	    GimGeometry.vec3f[] clipped_points1 = new GimGeometry.vec3f[GimTriCollision.MAX_TRI_CLIPPING];
	    GimGeometry.vec3f[] deep_points1 = new GimGeometry.vec3f[GimTriCollision.MAX_TRI_CLIPPING];
	    for (int i = 0; i < GimTriCollision.MAX_TRI_CLIPPING; i++) {
	    	_temp_clip[i] = new GimGeometry.vec3f(); //TODO is all this really necessary ??? TZ
...
//	    if (CLASSIFY_TRI_BY_FACE(tri2.m_vertices, tri1.m_planes.m_planes[0], _distances)) {
			...
			final GimDynArray<GimContact> contacts)
	{
	    ...
	    GimTriCollision.GIM_TRIANGLE_DATA tri1data = new GimTriCollision.GIM_TRIANGLE_DATA(), tri2data = new GimTriCollision.GIM_TRIANGLE_DATA();
	...
	static void gim_trimesh_plane_collision(final GimTrimesh trimesh,
                                            final GimGeometry.vec4f plane, final GimDynArray<GimGeometry.vec4f> contacts)
	{
	    contacts.m_size = 0;
	    ...
	    ObjArray<GimGeometry.vec3f> vertices = trimesh.m_transformed_vertex_buffer.GIM_BUFFER_ARRAY_POINTER(0);//vec3f,,0);

	    float dist;

	    for (i=0;i<vertcount;i++)
	    {
	        dist = GimGeometry.DISTANCE_PLANE_POINT(plane,vertices.at(i));
	        if(dist<=0.0f)
	        {
