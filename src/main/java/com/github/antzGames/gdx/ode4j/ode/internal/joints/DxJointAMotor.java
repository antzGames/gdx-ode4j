...
import com.github.antzGames.gdx.ode4j.ode.DAMotorJoint;
...
public class DxJointAMotor extends DxJoint implements DAMotorJoint
{
	...
	computeGlobalAxes( DVector3[] ax)//[3] )
	{
		if ( _mode == AMotorMode.dAMotorEuler )
		{
			...
			OdeMath.dMultiply0_331( ax[0], node[0].body.posr().R(), axis[0] );
			if ( node[1].body !=null)
			{
				OdeMath.dMultiply0_331( ax[2], node[1].body.posr().R(), axis[2] );
			}
			else
			{
//				ax[2].v[0] = axis[2].v[0];
...
//				ax[2].v[2] = axis[2].v[2];
		...
		else
		{
			for ( int i = 0; i < _num; i++ )
			{
				...
				{
					// relative to b1
					OdeMath.dMultiply0_331( ax[i], node[0].body.posr().R(), axis[i] );
				}
				else if ( _rel[i] == 2 )
				{
					// relative to b2
					...
					{
						OdeMath.dMultiply0_331( ax[i], node[1].body.posr().R(), axis[i] );
					}
	                else
	                {
	                    // global - just copy it
	...
	computeEulerAngles( DVector3[] ax) //[3] )
	{
		...
		OdeMath.dMultiply0_331( ref1, node[0].body.posr().R(), reference1 );
		if ( node[1].body!=null )
		{
			OdeMath.dMultiply0_331( ref2, node[1].body.posr().R(), reference2 );
		}
		else
		{
			ref2.set(reference2);
		...
		angle[0] = -dAtan2( OdeMath.dCalcVectorDot3( ax[2], q ), OdeMath.dCalcVectorDot3( ax[2], ref1 ) );

		// get q perpendicular to both ax[0] and ax[1], get second euler angle
		OdeMath.dCalcVectorCross3( q, ax[0], ax[1] );
		angle[1] = -dAtan2( OdeMath.dCalcVectorDot3( ax[2], ax[0] ), OdeMath.dCalcVectorDot3( ax[2], q ) );

		// get q perpendicular to both ax[1] and ax[2], get third euler angle
		OdeMath.dCalcVectorCross3( q, ax[1], ax[2] );
		angle[2] = -dAtan2( OdeMath.dCalcVectorDot3( ref2, ax[1] ), OdeMath.dCalcVectorDot3( ref2, q ) );
	}
	...
	setEulerReferenceVectors()
	{
		if ( node[0].body != null&& node[1].body != null)
		{
			...
			OdeMath.dMultiply0_331( r, node[1].body.posr().R(), axis[2] );
			OdeMath.dMultiply1_331( reference1, node[0].body.posr().R(), r );
			OdeMath.dMultiply0_331( r, node[0].body.posr().R(), axis[0] );
			OdeMath.dMultiply1_331( reference2, node[1].body.posr().R(), r );
		} else {
			// We want to handle angular motors attached to passive geoms
	        // Replace missing node.R with identity
	        if (node[0].body != null) {
	          OdeMath.dMultiply1_331( reference1, node[0].body.posr().R(), axis[2] );
	          OdeMath.dMultiply0_331( reference2, node[0].body.posr().R(), axis[0] );
	        } else if (node[1].body != null) {
	          OdeMath.dMultiply0_331( reference1, node[1].body.posr().R(), axis[2] );
	          OdeMath.dMultiply1_331( reference2, node[1].body.posr().R(), axis[0] );
	        }
		}
	}
	...
	public void dJointSetAMotorAxis( int anum, int rel, DVector3C r )
	{
		...
		if ( rel > 0 )
		{
			if ( rel == 1 )
			{
				OdeMath.dMultiply1_331( axis[anum], node[0].body.posr().R(), r );
			}
			else // rel == 2
			{
				...
				{
					OdeMath.dMultiply1_331( axis[anum], node[1].body.posr().R(), r );
				}
				else
				{
//					axis[anum].v[0] = r.v[0];
	...
	void dJointGetAMotorAxis( int anum, DVector3 result )
	{
	    ...
	    if ( _mode == AMotorMode.dAMotorEuler  ) {
	      ...
	      if (anum == 1) {
	        ...
	        OdeMath.dCalcVectorCross3( result, axes[1], axes[2] );
	      } else if (anum == 2) {
	        // Same problem as above.
	        OdeMath.dCalcVectorCross3( result, axes[0], axes[1] );
	      }
	    } else if ( _rel[anum] > 0 ) {
			if ( _rel[anum] == 1 )
			{
				OdeMath.dMultiply0_331( result, node[0].body.posr().R(), axis[anum] );
			}
			else
			{
				if ( node[1].body!=null )   // jds
				{
					OdeMath.dMultiply0_331( result, node[1].body.posr().R(), axis[anum] );
				}
				else
				{
