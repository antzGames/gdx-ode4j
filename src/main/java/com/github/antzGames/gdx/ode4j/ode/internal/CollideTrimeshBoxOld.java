...
import com.github.antzGames.gdx.ode4j.math.DVector4C;
import com.github.antzGames.gdx.ode4j.ode.DAABBC;
import com.github.antzGames.gdx.ode4j.ode.DColliderFn;
...
import com.github.antzGames.gdx.ode4j.ode.internal.cpp4j.java.RefBoolean;
...
import com.github.antzGames.gdx.ode4j.ode.internal.gimpact.GimDynArrayInt;
import com.github.antzGames.gdx.ode4j.ode.internal.gimpact.GimGeometry;
import com.github.antzGames.gdx.ode4j.ode.internal.gimpact.GimTrimesh;
...
public class CollideTrimeshBoxOld implements DColliderFn {
	...
	private class sTrimeshBoxColliderData
	{
				...
				DVector3 vNormal, int iAxis)
		{
			...
			if ( fMin <= Common.dEpsilon ) /// THIS NORMAL WOULD BE DANGEROUS
				return true;
		...
		boolean _cldTestSeparatingAxes(final DVector3C v0, final DVector3C v1, final DVector3C v2) {
			...
			fp1  = fp0;
			fp2  = fp0;
			fR=fa0* Common.dFabs( m_vN.dot(vA0) ) + fa1 * Common.dFabs( m_vN.dot(vA1) ) + fa2 * Common.dFabs( m_vN.dot(vA2) );
			...
			fR  = fa1 * Common.dFabs(vA2.dot(m_vE0)) + fa2 * Common.dFabs(vA1.dot(m_vE0));
			...
			fp2 = fp0;
			fR  = fa1 * Common.dFabs(vA2.dot(m_vE1)) + fa2 * Common.dFabs(vA1.dot(m_vE1));
			...
			fp1 = fp0 - vA0.dot(m_vN);
			fp2 = fp0 - vA0.dot(m_vN);
			fR  = fa1 * Common.dFabs(vA2.dot(m_vE2)) + fa2 * Common.dFabs(vA1.dot(m_vE2));
			...
			fR  = fa0 * Common.dFabs(vA2.dot(m_vE0)) + fa2 * Common.dFabs(vA0.dot(m_vE0));
			...
			fp2 = fp0;
			fR  = fa0 * Common.dFabs(vA2.dot(m_vE1)) + fa2 * Common.dFabs(vA0.dot(m_vE1));
			...
			fp1 = fp0 - vA1.dot(m_vN);
			fp2 = fp0 - vA1.dot(m_vN);
			fR  = fa0 * Common.dFabs(vA2.dot(m_vE2)) + fa2 * Common.dFabs(vA0.dot(m_vE2));
			...
			fR  = fa0 * Common.dFabs(vA1.dot(m_vE0)) + fa1 * Common.dFabs(vA0.dot(m_vE0));
			...
			fp2 = fp0;
			fR  = fa0 * Common.dFabs(vA1.dot(m_vE1)) + fa1 * Common.dFabs(vA0.dot(m_vE1));
			...
			fp1 = fp0 - vA2.dot(m_vN);
			fp2 = fp0 - vA2.dot(m_vN);
			fR  = fa0 * Common.dFabs(vA1.dot(m_vE2)) + fa1 * Common.dFabs(vA0.dot(m_vE2));
		...
		void _cldClipping(final DVector3C v0, final DVector3C v1, final DVector3C v2, final int TriIndex) {
			Common.dIASSERT( ((m_iFlags & OdeConstants.CONTACTS_UNIMPORTANT)==0) || m_ctContacts.i < (m_iFlags & DxGeom.NUMC_MASK) ); // Do not call the function if there is no room to store results
			...
			if (m_iBestAxis > 4 ) {
				...
				for ( int i=0; i<iTempCnt2.i; i++ ) {
					...
					if ((m_ctContacts.i | OdeConstants.CONTACTS_UNIMPORTANT) == (m_iFlags & (DxGeom.NUMC_MASK | OdeConstants.CONTACTS_UNIMPORTANT))) {
						break;
					}
				}
		...
		See also comments in GenerateContact()
			 */
			bOutFinishSearching.b = ((m_ctContacts.i | OdeConstants.CONTACTS_UNIMPORTANT) == (m_iFlags & (DxGeom.NUMC_MASK | OdeConstants.CONTACTS_UNIMPORTANT)));
	...
	int dCollideBTL(DxTriMesh g1, DxBox BoxGeom, int Flags, DContactGeomBuffer Contacts, int Stride)
	{
		...
		GimGeometry.vec3f[] dv = new GimGeometry.vec3f[] { new GimGeometry.vec3f(), new GimGeometry.vec3f(), new GimGeometry.vec3f() };//[3];
			...
			RefInt OutTriCount)
	{
		...
		do 
		{
			...
			if (0==(in_Flags & OdeConstants.CONTACTS_UNIMPORTANT))
			{
				...
				for (int i=0; i<OutTriCount.i; i++)
				{
					...
					if (diff.dot(diff) < Common.dEpsilon)
					{
						// same normal?
						if (1.0 - Common.dFabs(in_Normal.dot(Contact.normal)) < Common.dEpsilon)
						{
			...
			{
				Common.dIASSERT(OutTriCount.i < (in_Flags & DxGeom.NUMC_MASK));
			}
