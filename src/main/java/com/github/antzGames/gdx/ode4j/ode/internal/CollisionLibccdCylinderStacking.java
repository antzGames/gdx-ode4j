...
import com.github.antzGames.gdx.ode4j.ode.internal.libccd.CCDQuat;
import com.github.antzGames.gdx.ode4j.ode.internal.libccd.CCDVec3;

public class CollisionLibccdCylinderStacking {

	public static int collideCylCyl(DGeom o1, DGeom o2, CollisionLibccd.ccd_cyl_t cyl1, CollisionLibccd.ccd_cyl_t cyl2, int flags,
                                    DContactGeomBuffer contacts) {
		int maxContacts = Math.min(flags & DxGeom.NUMC_MASK, 8);
		if (maxContacts == 0) {
			return 0;
		}
		double axesProd = Math.abs(CCDVec3.ccdVec3Dot(cyl1.axis, cyl2.axis));
		...
		if (1.0 - axesProd < 1e-3) {
			...
			CollisionLibccd.ccd_cyl_t minCyl = r1isMin ? cyl1 : cyl2;
			CollisionLibccd.ccd_cyl_t maxCyl = r1isMin ? cyl2 : cyl1;
			CCDVec3.ccdVec3Copy(p, minCyl.pos);
			CCDVec3.ccdVec3Sub(p, maxCyl.pos);
			double dot = CCDVec3.ccdVec3Dot(p, maxCyl.axis);
			double lSum = l1 + l2;
			...
			// plane
			CCDVec3.ccd_vec3_t proj = new CCDVec3.ccd_vec3_t();
			CCDVec3.ccdVec3Copy(proj, maxCyl.axis);
			CCDVec3.ccdVec3Scale(proj, -dot);
			CCDVec3.ccdVec3Add(proj, p);
			double radiiDif = Math.sqrt(CCDVec3.ccdVec3Len2(proj));
			...
			// contact depth
			if (depth_v < depth_h) {
				double dot2 = -CCDVec3.ccdVec3Dot(p, minCyl.axis);
				...
				if (rmin + radiiDif <= rmax) {
					...
					CCDVec3.ccd_vec3_t proj2 = new CCDVec3.ccd_vec3_t();
					CCDVec3.ccdVec3Copy(proj2, proj);
					CCDQuat.ccdQuatRotVec(proj, maxCyl.rot_inv);
					...
					// placed at (0,0,0) now
					CCDVec3.ccdVec3Copy(proj, proj2);
					CCDVec3.ccdVec3Scale(proj, -1);
					CCDQuat.ccdQuatRotVec(proj, minCyl.rot_inv);
	...
	private static int testAndAddDiscContact(double angle, double radius, double length, double lSum, CollisionLibccd.ccd_cyl_t c1,
                                             CollisionLibccd.ccd_cyl_t maxCyl, double normaldir, DGeom o1, DGeom o2, DContactGeomBuffer contacts, int contactCount) {
		CCDVec3.ccd_vec3_t p = new CCDVec3.ccd_vec3_t();
		p.set(Math.cos(angle) * radius, Math.sin(angle) * radius, 0);
		CCDVec3.ccd_vec3_t p2 = new CCDVec3.ccd_vec3_t();
		// Transform to world coordinates
		CCDQuat.ccdQuatRotVec(p, c1.rot);
		CCDVec3.ccdVec3Add(p, c1.pos);
		CCDVec3.ccdVec3Copy(p2, p);
		CCDVec3.ccdVec3Sub(p2, maxCyl.pos);
		// Calculate contact depth
		double depth = lSum - Math.abs(CCDVec3.ccdVec3Dot(p2, maxCyl.axis));
		if (depth >= 0) {
			// Contact point
			CCDVec3.ccdVec3Copy(p2, c1.axis);
			CCDVec3.ccdVec3Scale(p2, length);
			CCDVec3.ccdVec3Add(p, p2);
	...
	private static int addContact(DGeom o1, DGeom o2, CCDVec3.ccd_vec3_t axis, DContactGeomBuffer contacts, CCDVec3.ccd_vec3_t p,
