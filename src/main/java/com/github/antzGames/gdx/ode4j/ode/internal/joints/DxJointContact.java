 ...
 *                                                                       *
 *************************************************************************/
package com.github.antzGames.gdx.ode4j.ode.internal.joints;
...
import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dPlaneSpace;
...
public class DxJointContact extends DxJoint implements DContactJoint
{
	...
	getInfo1( Info1 info )
	{
	    ...
	    if ( (contact.surface.mode & OdeConstants.dContactAxisDep) != 0 )
		{
			...
			if ( contact.surface.mu  == OdeConstants.dInfinity ) nub ++;
			if ( contact.surface.mu2 == OdeConstants.dInfinity ) nub ++;
	        if (roll) {
	            ...
	            if ( contact.surface.rho  == OdeConstants.dInfinity ) nub++;
	            if ( contact.surface.rho2 == OdeConstants.dInfinity ) nub++;
	            if ( contact.surface.rhoN == OdeConstants.dInfinity ) nub++;
	          }
		}
		else
		{
			if ( contact.surface.mu > 0 ) m += 2;
			if ( contact.surface.mu == OdeConstants.dInfinity ) nub += 2;
	        if (roll) {
	            if ( contact.surface.rho < 0 ) contact.surface.rho = 0;
	            else m+=3;
	            if ( contact.surface.rho == OdeConstants.dInfinity ) nub += 3;
	          }
		}
...
//			c2.v[2] = contact.geom.pos.v[2] - node[1].body._posr.pos.v[2];
		...
		double erp = worldERP;
		if (( contact.surface.mode & OdeConstants.dContactSoftERP ) != 0)
			erp = contact.surface.soft_erp;
		double k = worldFPS * erp;
		double depth = contact.geom.depth - world.contactp.min_depth;
		if ( depth < 0 ) depth = 0;

		if (( contact.surface.mode & OdeConstants.dContactSoftCFM ) != 0)
			info.setCfm(rowNormal, contact.surface.soft_cfm);


		double motionN = 0;
		if (( contact.surface.mode & OdeConstants.dContactMotionN ) != 0)
		...
		// deal with bounce
		if (( contact.surface.mode & OdeConstants.dContactBounce) != 0)
		{
		...
		info.setLo(0, 0);
		info.setHi(0, OdeConstants.dInfinity);
		...
		if (( contact.surface.mode & OdeConstants.dContactFDir1) != 0)   // use fdir1 ?
		{
...
//				t1.v[2] = contact.fdir1.v[2];
	    ...
	    if ( contact.surface.mu > 0 )
	    {
	    	...
	    	// set right hand side
	    	if (( contact.surface.mode & OdeConstants.dContactMotion1) != 0)
	    	{
	    	...
	    	if (( contact.surface.mode & OdeConstants.dContactApprox1_1) != 0)
	    		//info._findexA[info._findexP+1] = 0;//info.findex[1] = 0;
	    		info.setFindex(rowFriction1, 0);

	    	// set slip (constraint force mixing)
	    	if (( contact.surface.mode & OdeConstants.dContactSlip1) != 0)
	    ...
	    final double mu2 = (contact.surface.mode & OdeConstants.dContactMu2)!=0 ? contact.surface.mu2 : contact.surface.mu;
...
//				dNegateVector3( J2a_plus_s2 );
			...
			// set right hand side
			if (( contact.surface.mode & OdeConstants.dContactMotion2) != 0)
			{
			...
			if (( contact.surface.mode & OdeConstants.dContactApprox1_2) != 0)
				info.setFindex(rowFriction2, 0);//info.findex[2] = 0;

			// set slip (constraint force mixing)
			if (( contact.surface.mode & OdeConstants.dContactSlip2) != 0)
	    ...
	    if ((contact.surface.mode&OdeConstants.dContactRolling)!=0) {
	        ...
	        approx[0] = contact.surface.mode & OdeConstants.dContactApprox1_1;
	        approx[1] = contact.surface.mode & OdeConstants.dContactApprox1_2;
	        approx[2] = contact.surface.mode & OdeConstants.dContactApprox1_N;
