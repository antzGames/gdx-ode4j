...
public class GimAABBSet { //Formerly GimBoxPruning
	...
	int m_count;
	GimGeometry.aabb3f m_global_bound = new GimGeometry.aabb3f();//!< Global calculated bound of all boxes
	private final GimGeometry.aabb3f[] m_boxes;// = new aabb3f[0];  //TZ Why init
	long[] m_maxcoords;//!<Upper corners of the boxes, in integer representation
	GimRadixSort.GIM_RSORT_TOKEN[] m_sorted_mincoords;//!< sorted min coords (lower corners), with their coord value as the m_key and m_value as the box index
	char m_shared;//!< if m_shared == 0 then the memory is allocated and the set must be destroyed, else the pointers are shared and the set should't be destroyed
	//	};
...
//	}
	...
	long GIM_CONVERT_VEC3F_GUINT_XZ_CLAMPED(float vx, float vz)
	{
		...
		//	    uint_key = (uint_key<<16) + _z;
		float _cx = GimMath.CLAMP(vx,-MAX_AABB_SIZE,MAX_AABB_SIZE);
		float _cz = GimMath.CLAMP(vz,-MAX_AABB_SIZE,MAX_AABB_SIZE);
	...
	private GimAABBSet(int count) {
		//private!!
		m_boxes = new GimGeometry.aabb3f[count];
	}
	...
	static GimAABBSet gim_aabbset_alloc(int count)
	{
		...
		for (int i = 0; i < count; i++) x.m_boxes[i] = new GimGeometry.aabb3f(); //TZ
		...
		else
		{
			...
			for (int i = 0; i < x.m_count; i++) x.m_sorted_mincoords[i] = new GimRadixSort.GIM_RSORT_TOKEN();
		}
		x.m_shared = 0;
...
//		}
		...
		GimGeometry.AABB_COPY(m_global_bound, m_boxes[0]);
		//check the first one again, but whatever.
		for (GimGeometry.aabb3f aabb: m_boxes) {
			GimGeometry.MERGEBOXES(m_global_bound, aabb);
		}
	}
	...
	void gim_aabbset_sort(final boolean calc_global_bound)
	{
		...
		{//allocate
			...
			m_sorted_mincoords = new GimRadixSort.GIM_RSORT_TOKEN[m_count];//(GIM_RSORT_TOKEN *)gim_alloc(sizeof(GIM_RSORT_TOKEN)*aabbset.m_count);
		}

		int i, count = m_count;
		GimGeometry.aabb3f[] paabb = m_boxes;
		long[] maxcoords = m_maxcoords;
		GimRadixSort.GIM_RSORT_TOKEN[] sorted_tokens = m_sorted_mincoords;
		...
		else
		{
			...
			GimRadixSort.GIM_RSORT_TOKEN[] unsorted = new GimRadixSort.GIM_RSORT_TOKEN[count]; //TZ TODO clean up
			//Calculate keys
			for(i=0;i<count;i++)
			{
				unsorted[i] = new GimRadixSort.GIM_RSORT_TOKEN(); //TODO optimize! E.g. use two arrays?
				maxcoords[i] = GIM_CONVERT_VEC3F_GUINT_XZ_UPPER(paabb[i].maxX,paabb[i].maxZ);
...
//    };
	...
	static void FIND_OVERLAPPING_FOWARD(
			...
			final GimGeometry.aabb3f test_aabb,
			final long max_coord_uint,
			final GimRadixSort.GIM_RSORT_TOKEN[] sorted_tokensA,
			final int sorted_tokensP,
			final GimGeometry.aabb3f[] aabbarray,
			...
			final PushPairMacro push_pair_macro)
	{
		...
		{
			_intersected = GimGeometry.AABBCOLLISION(test_aabb,aabbarray[_psorted_tokensA[_psorted_tokensP].m_value]);
			if(_intersected)
			{
			...
			final GimDynArray<GIM_PAIR> collision_pairs)
	{
		...
		intersected = GimGeometry.AABBCOLLISION(aabbset1.m_global_bound,aabbset2.m_global_bound);
		...
		//Clasify set 1
		GimRadixSort.GIM_RSORT_TOKEN[] classified_tokens1 = new GimRadixSort.GIM_RSORT_TOKEN[count1];//(GIM_RSORT_TOKEN *) gim_alloc(sizeof(GIM_RSORT_TOKEN)*count1);
...
//		System.out.println("IGNORED!");//TODO fix this!
	...
	void gim_aabbset_ray_collision(GimGeometry.vec3f vorigin, GimGeometry.vec3f vdir, float tmax,
