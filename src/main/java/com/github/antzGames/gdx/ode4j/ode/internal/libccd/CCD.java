...
import com.github.antzGames.gdx.ode4j.ode.internal.cpp4j.java.Ref;
import com.github.antzGames.gdx.ode4j.ode.internal.libccd.CCDPolyTope.ccd_pt_edge_t;
import com.github.antzGames.gdx.ode4j.ode.internal.libccd.CCDPolyTope.ccd_pt_face_t;
import com.github.antzGames.gdx.ode4j.ode.internal.libccd.CCDSimplex.ccd_simplex_t;

import java.util.Arrays;

import static com.github.antzGames.gdx.ode4j.ode.internal.libccd.CCDPolyTope.*;
...
public class CCD {
	...
	public static interface ccd_support_fn {
		void run(final Object obj, final CCDVec3.ccd_vec3_t dir, CCDVec3.ccd_vec3_t vec);
	}
	...
	static interface ccd_first_dir_fn {
		void run(final Object obj1, final Object obj2, CCDVec3.ccd_vec3_t dir);
	}
	...
	public static interface ccd_center_fn {
		void run(final Object obj1, CCDVec3.ccd_vec3_t center);
	}
	 ...
	 */
	static void ccdFirstDirDefault(final Object o1, final Object o2, CCDVec3.ccd_vec3_t dir)
	{
		CCDVec3.ccdVec3Set(dir, CCDVec3.CCD_ONE, CCDVec3.CCD_ZERO, CCDVec3.CCD_ZERO);
	}
	...
	public static int ccdGJKSeparate(final Object obj1, final Object obj2, final ccd_t ccd,
			CCDVec3.ccd_vec3_t sep)
	{
		ccd_pt_t polytope = new ccd_pt_t();
		Ref<ccd_pt_el_t<?>> nearestRef = new Ref<ccd_pt_el_t<?>>();
		int ret;
...
//		ccdListForEachEntry(pt.vertices, v, ccd_pt_vertex_t, list){
		...
		CCDVec3.ccdVec3Set(pos, CCDVec3.CCD_ZERO, CCDVec3.CCD_ZERO, CCDVec3.CCD_ZERO);
		scale = CCDVec3.CCD_ZERO;
		if ((len & 1) == 1)
			len++;

		for (i = 0; i < len / 2; i++){
			CCDVec3.ccdVec3Add(pos, vs[i].v.v1);
			CCDVec3.ccdVec3Add(pos, vs[i].v.v2);
			scale += (2.);
		}
		CCDVec3.ccdVec3Scale(pos, CCDVec3.CCD_ONE / scale);
	...
	{
		ccd_pt_t polytope = new ccd_pt_t();
		final Ref<ccd_pt_el_t<?>> nearestRef = new Ref<ccd_pt_el_t<?>>();
		int ret;
	...
	{
		long iterations;
		CCDVec3.ccd_vec3_t dir = new CCDVec3.ccd_vec3_t(); // direction vector
		CCDSupport.ccd_support_t last = new CCDSupport.ccd_support_t(); // last support point
		int do_simplex_res;
		...
		for (iterations = 0L; iterations < ccd.max_iterations; ++iterations) {
			...
			if (CCDVec3.ccdVec3Dot(last.v, dir) < CCDVec3.CCD_ZERO){
	...
	{
		ccd_simplex_t simplex = new ccd_simplex_t();
		CCDSupport.ccd_support_t supp = new CCDSupport.ccd_support_t(); // support point
		int ret, size;
	...
	private static int doSimplex2(ccd_simplex_t simplex, CCDVec3.ccd_vec3_t dir)
	{
		final CCDSupport.ccd_support_t A, B;
		CCDVec3.ccd_vec3_t AB = new CCDVec3.ccd_vec3_t(), AO = new CCDVec3.ccd_vec3_t(), tmp = new CCDVec3.ccd_vec3_t();
		double dot;
		...
		CCDVec3.ccdVec3Scale(AO, -CCDVec3.CCD_ONE);
		...
		if (CCDVec3.ccdIsZero(CCDVec3.ccdVec3Len2(tmp)) && dot > CCDVec3.CCD_ZERO){
			return 1;
		}
	 ...
	 *  This function also alteres simplex and dir according to further
	 *  processing of GJK algorithm. */
	private static int doSimplex3(ccd_simplex_t simplex, CCDVec3.ccd_vec3_t dir)
	{
		final CCDSupport.ccd_support_t A, B, C;
		final CCDVec3.ccd_vec3_t AO = new CCDVec3.ccd_vec3_t(), AB = new CCDVec3.ccd_vec3_t(), AC = new CCDVec3.ccd_vec3_t();
		final CCDVec3.ccd_vec3_t ABC = new CCDVec3.ccd_vec3_t(), tmp = new CCDVec3.ccd_vec3_t();
		double dot, dist;
	 ...
	 *  This function also alteres simplex and dir according to further
	 *  processing of GJK algorithm. */
	private static int doSimplex4(ccd_simplex_t simplex, CCDVec3.ccd_vec3_t dir)
	{
		final CCDSupport.ccd_support_t A, B, C, D;
		CCDVec3.ccd_vec3_t AO = new CCDVec3.ccd_vec3_t(), AB = new CCDVec3.ccd_vec3_t(), AC = new CCDVec3.ccd_vec3_t();
		CCDVec3.ccd_vec3_t AD = new CCDVec3.ccd_vec3_t();
		CCDVec3.ccd_vec3_t ABC = new CCDVec3.ccd_vec3_t(), ACD = new CCDVec3.ccd_vec3_t(), ADB = new CCDVec3.ccd_vec3_t();
		...
		// found
		dist = CCDVec3.ccdVec3PointTriDist2(A.v, B.v, C.v, D.v, null);
		if (CCDVec3.ccdIsZero(dist)){
			return -1;
		}
		...
		B_on_ACD = CCDVec3.ccdSign(CCDVec3.ccdVec3Dot(ACD, AB));
		C_on_ADB = CCDVec3.ccdSign(CCDVec3.ccdVec3Dot(ADB, AC));
		D_on_ABC = CCDVec3.ccdSign(CCDVec3.ccdVec3Dot(ABC, AD));

		// whether origin is on same side of ACD, ADB, ABC as B, C, D
		// respectively
		AB_O = CCDVec3.ccdSign(CCDVec3.ccdVec3Dot(ACD, AO)) == B_on_ACD;
		AC_O = CCDVec3.ccdSign(CCDVec3.ccdVec3Dot(ADB, AO)) == C_on_ADB;
		AD_O = CCDVec3.ccdSign(CCDVec3.ccdVec3Dot(ABC, AO)) == D_on_ABC;
	 ...
	 *  This function also alteres simplex and dir according to further
	 *  processing of GJK algorithm. */
	private static int doSimplex(ccd_simplex_t simplex, CCDVec3.ccd_vec3_t dir)
	{
	...
	/** d = a x b x c */
	private static void tripleCross(final CCDVec3.ccd_vec3_t a, final CCDVec3.ccd_vec3_t b,
                                    final CCDVec3.ccd_vec3_t c, CCDVec3.ccd_vec3_t d)
	{
...
//		}
	...
	{
		final CCDSupport.ccd_support_t a, b, c;
		final CCDSupport.ccd_support_t d = new CCDSupport.ccd_support_t(), d2 = new CCDSupport.ccd_support_t();
		final CCDVec3.ccd_vec3_t ab = new CCDVec3.ccd_vec3_t(), ac = new CCDVec3.ccd_vec3_t(), dir = new CCDVec3.ccd_vec3_t();
		...
		dist = CCDVec3.ccdVec3PointTriDist2(d.v, a.v, b.v, c.v, null);

		// and second one take in opposite direction
		CCDVec3.ccdVec3Scale(dir, -CCDVec3.CCD_ONE);
		CCDSupport.__ccdSupport(obj1, obj2, dir, ccd, d2);
		dist2 = CCDVec3.ccdVec3PointTriDist2(d2.v, a.v, b.v, c.v, null);
	...
	{
		final CCDSupport.ccd_support_t a, b;
		CCDVec3.ccd_vec3_t ab = new CCDVec3.ccd_vec3_t(), ac = new CCDVec3.ccd_vec3_t(), dir = new CCDVec3.ccd_vec3_t();
		CCDSupport.ccd_support_t[] supp = new CCDSupport.ccd_support_t[4];
		for (int i = 0; i < supp.length; i++) supp[i] = new CCDSupport.ccd_support_t();
		...
		found = false;
		for (i = 0; i < CCDVec3.ccd_points_on_sphere_len; i++){
			CCDSupport.__ccdSupport(obj1, obj2, CCDVec3.ccd_points_on_sphere[i], ccd, supp[0]);
	...
	static void expandPolytope(ccd_pt_t pt, final ccd_pt_el_t<?> el,
			final CCDSupport.ccd_support_t newv)
	{
	...
	static int nextSupport(final Object obj1, final Object obj2, final ccd_t ccd,
			...
			final CCDSupport.ccd_support_t out)
	{
		CCDVec3.ccd_vec3_t a, b, c;
		double dist;
