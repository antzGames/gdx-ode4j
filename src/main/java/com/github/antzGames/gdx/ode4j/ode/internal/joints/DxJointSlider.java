...
import com.github.antzGames.gdx.ode4j.ode.DSliderJoint;
import com.github.antzGames.gdx.ode4j.ode.OdeConstants;
import com.github.antzGames.gdx.ode4j.ode.OdeMath;
import com.github.antzGames.gdx.ode4j.ode.internal.Common;
...
public class DxJointSlider extends DxJoint implements DSliderJoint
{
	...
	public double dJointGetSliderPosition ( )
	{
		...
		OdeMath.dMultiply0_331 ( ax1, node[0].body.posr().R(), axis1 );

		if ( node[1].body!= null )
		{
			// get body2 + offset point in global coordinates
			OdeMath.dMultiply0_331 ( q, node[1].body.posr().R(), offset );
//			for ( int i = 0; i < 3; i++ )
...
//				ax1.v[2] = -ax1.v[2];
	...
	public double dJointGetSliderPositionRate ( )
	{
		...
		if ( node[1].body != null )
		{
			return OdeMath.dCalcVectorDot3 ( ax1, node[0].body.lvel ) -
			OdeMath.dCalcVectorDot3 ( ax1, node[1].body.lvel );
		}
		else
		{
			double rate = OdeMath.dCalcVectorDot3 ( ax1, node[0].body.lvel );
	...
	getInfo1 ( Info1 info )
	{
		...
		limot.limit = 0;
		if ( ( limot.lostop > -OdeConstants.dInfinity || limot.histop < OdeConstants.dInfinity ) &&
	...
	getInfo2 ( double worldFPS, double worldERP, Info2Descr info )
	{
		...
		OdeMath.dMultiply0_331 ( ax1, R1, axis1 );
		...
		if ( node[1].body != null)
		{
			...
			OdeMath.dMultiply0_331 ( ofs, R2, offset );
...
//			c.v[2] = 0.5 * ( joint.node[1].body._posr.pos.v[2] - joint.node[0].body._posr.pos.v[2] );
	...
	computeInitialRelativeRotation()
	{
		if ( node[0].body != null)
		{
			...
			if ( node[1].body != null )
			{
				Rotation.dQMultiply1 ( qrel, node[0].body._q, node[1].body._q );
			}
			else
			{
...
//	        c[2] = node[0].body->posr.pos[2] - node[1].body->posr.pos[2];
	        ...
	        OdeMath.dMultiply1_331 ( offset, node[1].body.posr().R(), c );
	    }
