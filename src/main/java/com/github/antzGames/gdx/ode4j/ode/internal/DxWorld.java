...
import com.github.antzGames.gdx.ode4j.ode.internal.cpp4j.java.RefInt;
import com.github.antzGames.gdx.ode4j.ode.internal.joints.DxJoint;
import com.github.antzGames.gdx.ode4j.ode.internal.processmem.DxIslandsProcessingCallContext;
import com.github.antzGames.gdx.ode4j.ode.internal.processmem.DxStepWorkingMemory;
...
import com.github.antzGames.gdx.ode4j.ode.internal.processmem.DxWorldProcessContext;
...
import com.github.antzGames.gdx.ode4j.ode.internal.processmem.DxWorldProcessMemoryManager;
import com.github.antzGames.gdx.ode4j.ode.internal.processmem.DxWorldProcessMemoryReserveInfo;
import com.github.antzGames.gdx.ode4j.ode.threading.Threading;
import com.github.antzGames.gdx.ode4j.ode.threading.task.SameThreadTaskExecutor;
import com.github.antzGames.gdx.ode4j.ode.threading.task.Task;
import com.github.antzGames.gdx.ode4j.ode.threading.task.TaskGroup;
import com.github.antzGames.gdx.ode4j.ode.DWorld;
import com.github.antzGames.gdx.ode4j.ode.threading.task.TaskExecutor;

import static com.github.antzGames.gdx.ode4j.ode.OdeMath.dCalcVectorDot3;
import static com.github.antzGames.gdx.ode4j.ode.internal.Common.dUASSERT;
...
public class DxWorld extends DBase implements DWorld {
	...
	Objects_H.dxAutoDisable adis;		// auto-disable parameters
	...
	Objects_H.dxDampingParameters dampingp; // damping parameters
...
//    dxThreadingFunctionsInfo RetrieveThreadingDefaultImpl(dThreadingImplementationID &out_default_impl);
	...
	private DxWorld() {
		...
		contactp = null;
		dampingp = null;
		max_angular_speed = OdeConstants.dInfinity;
		userdata = 0;

	    //dSetZero (gravity, 4);
		gravity = new DVector3();

		adis = new Objects_H.dxAutoDisable();
		qs = new Objects_H.dxQuickStepParameters();
		contactp = new Objects_H.dxContactParameters();
		dampingp = new Objects_H.dxDampingParameters();
	}
...
//		dAASSERT (w);
		...
		while (j != null) {
			...
			if ( j.isFlagsInGroup() ) {
				...
				j.node[1].body = null;
				j.node[1].next = null;
				ErrorHandler.dMessage (0,"warning: destroying world containing grouped joints");
			}
			else {
...
//	}
	...
	{
	    Common.dUASSERT (policyinfo==null || (policyinfo.struct_size >= DxUtil.sizeof(policyinfo) && policyinfo.reserve_factor >= 1.0f), "Bad policy info");
	...
	{
	    Common.dUASSERT (stepsize > 0,"stepsize must be > 0");
...
//		force.v[2] = stepsize * iz;
	...
	{
		return Common.dSqrt (adis.linear_average_threshold);
	}
	...
	{
		return Common.dSqrt (adis.angular_average_threshold);
	}
	...
	{
		return Common.dSqrt(dampingp.linear_threshold);
	}
	...
	{
		return Common.dSqrt(dampingp.angular_threshold);
	}
...
//	public void dWorldSetMaxAngularSpeed(dxWorld w, double max_speed)
	...
	{
		//dAASSERT(w);
		if (max_speed < OdeConstants.dInfinity)
...
//	    return pctCurrentInstance;
	...
	static int EstimateIslandProcessingSimultaneousCallsMaximumCount(int activeThreadCount,
			...
			int stepperAllowedThreadCount, DxStepperProcessingCallContext.dmaxcallcountestimate_fn_t maxCallCountEstimator)
	{
	...
	boolean dxProcessIslands (final DxWorldProcessIslandsInfo islandsInfo,
			double stepSize, DxStepperProcessingCallContext.dstepper_fn_t stepper,
			DxStepperProcessingCallContext.dmaxcallcountestimate_fn_t maxCallCountEstimator)
	{
		...
		Common.dIASSERT(islandsAllowedThreadCount != 0);
		Common.dIASSERT(activeThreadCount.get() >= islandsAllowedThreadCount);
...
//					average_avel.v[2] *= r1;
				...
				av_lspeed = OdeMath.dCalcVectorDot3( average_lvel, average_lvel );
				...
				{
					av_aspeed = OdeMath.dCalcVectorDot3( average_avel, average_avel );
...
//	* @see DWorld#setStepMemoryManager(DWorldStepMemoryFunctionsInfo)
	...
	*/
    ...
    public static class DWorldStepMemoryFunctionsInfo
	{
	    ...
	    public static final DxUtil.alloc_block_fn_t alloc_block = new DxUtil.alloc_block_fn_t() {
			@Override
			public DxUtil.BlockPointer run(int block_size) {
	    ...
	    public static final DxUtil.shrink_block_fn_t shrink_block = new DxUtil.shrink_block_fn_t() {
			@Override
			public DxUtil.BlockPointer run(DxUtil.BlockPointer block_pointer, int block_current_size,
	    ...
	    public static final DxUtil.free_block_fn_t free_block = new DxUtil.free_block_fn_t() {
			@Override
			public void run(DxUtil.BlockPointer block_pointer, int block_current_size) {
				//TODO?
			}
		};
	};
